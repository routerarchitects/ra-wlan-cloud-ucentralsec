...
namespace OpenWifi {
	...
	bool BaseUserDB::CreateUser([[maybe_unused]] const std::string &Admin,
								SecurityObjects::UserInfo &NewUser, bool PasswordHashedAlready) {
		try {
			...
			if (NewUser.currentPassword.empty()) {
				...
				if (!PasswordHashedAlready) {
					NewUser.currentPassword = AuthService()->ComputeNewPasswordHash(
						NewUser.email, NewUser.currentPassword);
	...
	bool BaseUserDB::GetUsers(uint64_t Offset, uint64_t HowMany,
							  SecurityObjects::UserInfoVec &Users, const std::string &WhereClause) {
		try {
	...
	bool BaseUserDB::UpdateUserInfo([[maybe_unused]] const std::string &Admin,
									SecurityObjects::USER_ID_TYPE &Id,
	...
	bool BaseUserDB::DeleteUser([[maybe_unused]] const std::string &Admin,
								const SecurityObjects::USER_ID_TYPE &Id) {
		return DeleteRecord("id", Id);
	}

	bool BaseUserDB::DeleteUsers([[maybe_unused]] const std::string &Admin,
	...
	bool BaseUserDB::SetLastLogin(const std::string &Id) {
		try {
			...
			Update << ConvertParams(St1), Poco::Data::Keywords::use(Now),
	...
	UserCache::UserCache(unsigned Size, unsigned TimeOut, bool Users)
		...
		CacheById_ = std::make_unique<Poco::ExpireLRUCache<std::string, SecurityObjects::UserInfo>>(
			Size, TimeOut);
		CacheByEMail_ =
			std::make_unique<Poco::ExpireLRUCache<std::string, std::string>>(Size, TimeOut);
	}
	...
	inline bool UserCache::GetFromCache(const std::string &FieldName, const std::string &Value,
										SecurityObjects::UserInfo &R) {
...
template <>
void ORM::DB<OpenWifi::UserInfoRecordTuple, OpenWifi::SecurityObjects::UserInfo>::Convert(
	const OpenWifi::UserInfoRecordTuple &T, OpenWifi::SecurityObjects::UserInfo &U) {
	U.id = T.get<0>();
	U.name = T.get<1>();
...
template <>
void ORM::DB<OpenWifi::UserInfoRecordTuple, OpenWifi::SecurityObjects::UserInfo>::Convert(
	const OpenWifi::SecurityObjects::UserInfo &U, OpenWifi::UserInfoRecordTuple &T) {
	T.set<0>(U.id);
	T.set<1>(U.name);
